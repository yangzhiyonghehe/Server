...
func AddDeviceEmployee(deviceInfo my_db.DeviceInfo, employeeInfo EmployeeInfo) error {
	...
	infoJson.CustomizeID = employeeInfo.UserID
	infoJson.IdType = 0
...
func RemoveDeviceEmployee(deviceInfo my_db.DeviceInfo, employeeInfo EmployeeInfo) error {
	...
	if deviceInfo.Type == 0 {
		...
		infoIndex.CustomizeID = append(infoIndex.CustomizeID, employeeInfo.UserID)
...
func RemoveAllDeviceEmployee(deviceInfo my_db.DeviceInfo) error {
	mapRequest := make(map[string]interface{}, 0)

	mapRequest["operator"] = "DeleteAllPerson"

	mapInfo := make(map[string]interface{}, 0)
	mapInfo["DeleteAllPersonCheck"] = 1
	mapRequest["info"] = mapInfo

	//mapRequest["info"] =

	bytePost, err := json.Marshal(mapRequest)
	...
	err = json.Unmarshal(respBytes, &mapReply)
	if err != nil {
		return err
	}

	if mapReply["code"].(float64) != 200 {
		mapInfo = mapReply["info"].(map[string]interface{})
		return errors.New(mapInfo["Detail"].(string))
	}
